...
func TestHexConversion(t *testing.T) {
    for i, tt := range vals {
        hex := tt.c.Hex()
        if hex != tt.hexGenerated {
            t.Errorf("%v. %v.Hex() => (%v), want %v (delta %v)", i, tt.c, hex, tt.hexGenerated, delta)
        }
    }
}
